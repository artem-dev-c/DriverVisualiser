 ...
 *
 * Retrieves error/warning/information events from System and Configuration logs
 * for a configurable number of days (default: 7) from driver-related providers
 * (Kernel-PnP, DriverFrameworks-UserMode, UserPnP).
 * Matches events to specific drivers using Instance ID.
 */
class ErrorLogReader {
public:
    /// Query System + Configuration logs for all driver-related events (MULTI-LOG)
    ...
    /// Extract EventData field from XML (used by event message builder and parsing)
    static std::optional<std::wstring> extractEventDataField(
        const std::wstring& xml, 
        const std::wstring& fieldName
    );

    // XML parsing helpers (made public for use by static helper functions)
